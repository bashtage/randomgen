/*
        A C++ version of Jean-Philippe Aumasson's BlaBla PRNG (modified from
   Orson Peters 'chacha.h' original code)
...
template <uint32_t R = 10> class BlaBla {
public:
  typedef uint64_t result_type;

  explicit BlaBla(uint64_t seedval = 0x1ee106e9041096e4,
                  uint64_t stream = 0x037926afc39dcbd9);
  explicit BlaBla(uint64_t seedval[2], uint64_t stream[2]);
  template <class Sseq> explicit BlaBla(Sseq &seq);

  void seed(uint64_t seedval = 0x1ee106e9041096e4,
            uint64_t stream = 0x037926afc39dcbd9);
  ...
  template <uint32_t R_>
  friend bool operator==(const BlaBla<R_> &lhs, const BlaBla<R_> &rhs);
  template <uint32_t R_>
  friend bool operator!=(const BlaBla<R_> &lhs, const BlaBla<R_> &rhs);

  template <uint32_t R_, typename CharT, typename Traits>
  friend std::basic_ostream<CharT, Traits> &
  operator<<(std::basic_ostream<CharT, Traits> &os, const BlaBla<R_> &rng);

  template <uint32_t R_, typename CharT, typename Traits>
  friend std::basic_istream<CharT, Traits> &
  operator>>(std::basic_istream<CharT, Traits> &is, BlaBla<R_> &rng);
...
template <uint32_t R> inline uint64_t BlaBla<R>::operator()() {
...
template <uint32_t R> inline void BlaBla<R>::discard(unsigned long long n) {
  ctr += n;
}

template <uint32_t R> inline void BlaBla<R>::generate_block() {
  uint64_t constants[4] = {0x6170786593810fab, 0x3320646ec7398aee,
                           0x79622d3217318274, 0x6b206574babadada};
...
#ifdef __AVX2__
template <uint32_t R> inline void BlaBla<R>::blabla_core() {
#define _mm256_roti_epi64(r, c)                                                \
  _mm256_xor_si256(_mm256_srli_epi64((r), (c)),                                \
                   _mm256_slli_epi64((r), 64 - (c)))
...
#define mix_func(a, b, c, d)                                                   \
  block[a] += block[b];                                                        \
  block[d] ^= block[a];                                                        \
  block[d] = rotate_right(block[d], 32);                                       \
  block[c] += block[d];                                                        \
  block[b] ^= block[c];                                                        \
  block[b] = rotate_right(block[b], 24);                                       \
  block[a] += block[b];                                                        \
  block[d] ^= block[a];                                                        \
  block[d] = rotate_right(block[d], 16);                                       \
  block[c] += block[d];                                                        \
  block[b] ^= block[c];                                                        \
  block[b] = rotate_right(block[b], 63);
...
inline bool operator==(const BlaBla<R> &lhs, const BlaBla<R> &rhs) {
  for (uint32_t i = 0; i < 4; ++i) {
    if (lhs.keysetup[i] != rhs.keysetup[i])
      return false;
  }
...
operator<<(std::basic_ostream<CharT, Traits> &os, const BlaBla<R> &rng) {
...
operator>>(std::basic_istream<CharT, Traits> &is, BlaBla<R> &rng) {

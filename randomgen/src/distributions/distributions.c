...
RAND_INT_TYPE random_binomial_inversion(brng_t *brng_state, RAND_INT_TYPE n,
  ...
  if (!(binomial->has_binomial) || (binomial->nsave != n) ||
    ...
    qn = binomial->r;
    np = binomial->c;
    bound = (RAND_INT_TYPE)binomial->m;
  }
  X = 0;
  px = qn;
  U = next_double(brng_state);
...
uint64_t random_bounded_uint64(brng_t *brng_state, uint64_t off, uint64_t rng,
  ...
  if (rng == 0) {
    ...
    if (use_masked) {
      return off +
             buffered_bounded_masked_uint32(brng_state, (uint32_t)rng,
                                            (uint32_t)mask, NULL, NULL);
    } else {
      return off + buffered_bounded_lemire_uint32(brng_state, (uint32_t)rng,
                                                  NULL, NULL);
    }
...
void random_bounded_uint64_fill(brng_t *brng_state, uint64_t off, uint64_t rng,
  ...
  if (rng == 0) {
    ...
    if (use_masked) {
      ...
      for (i = 0; i < cnt; i++) {
        out[i] = off + buffered_bounded_masked_uint32(brng_state, (uint32_t)rng,
                                                      (uint32_t)mask, &bcnt, &buf);
      }
    } else {
      for (i = 0; i < cnt; i++) {
        out[i] =
            off + (uint64_t)buffered_bounded_lemire_uint32(brng_state, (uint32_t)rng,
                                                           &bcnt, &buf);
      }
    }
  } else if (rng == 0xFFFFFFFFFFFFFFFFULL) {
